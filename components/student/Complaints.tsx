...
import { ComplaintStatus, Sentiment } from '../../types';

// Mock logged-in student
const LOGGED_IN_STUDENT = "R. Kumar";
...
const Complaints: React.FC = () => {
    ...
    // Use a state to force re-render when a new complaint is added
    const [myComplaints, setMyComplaints] = useState(complaintsData.filter(c => c.submittedBy === LOGGED_IN_STUDENT));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        alert(`Complaint submitted and categorized as "${sentiment}".`);
        setTitle('');
        setDetails('');
    }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg mb-8">
         <div>
          ...
          <input 
            ...
            placeholder="e.g., Broken chair in classroom 302" required
          />
        </div>
        <div>
      ...
      <h3 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">My Past Complaints</h3>
          ...
          <div key={complaint.id} className="p-4 bg-gray-100 dark:bg-gray-700/50 rounded-lg">
             <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Submitted on: {complaint.date}</p>
                </div>
