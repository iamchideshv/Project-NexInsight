...
const FacultyTimeTable: React.FC = () => {
  ...
  const getCellColor = (period: Period | null) => {
    if (!period || !period.faculty.includes(LOGGED_IN_FACULTY)) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white">📅 My Timetable</h2>
        <p className="text-gray-600 dark:text-gray-400">Showing classes for: <strong>{LOGGED_IN_FACULTY}</strong></p>
      </div>
            ...
            <React.Fragment key={day}>
              ...
              {(schedule as any)[day].map((period: Period | null, index: number) => (
                <div key={`${day}-${index}`} className={`p-2 rounded-lg m-1 text-center flex flex-col justify-center ${getCellColor(period)}`}>
                  {period && period.faculty.includes(LOGGED_IN_FACULTY) ? (
                    <>
